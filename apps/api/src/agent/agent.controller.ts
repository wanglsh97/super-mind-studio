...
} from './files/document-file-policy';
import {
...
export class AgentController {
  constructor(
    ...
    @Inject(AgentExecutionSessionService) private readonly sessions: AgentExecutionSessionService,
  ) {}
  ...
  @UseInterceptors(
    FilesInterceptor('files', MAX_DOCUMENT_FILE_COUNT, {
      limits: { fileSize: MAX_DOCUMENT_FILE_BYTES },
    }),
  )
  async uploadThreadFiles(
    @Param('threadId', ParseUUIDPipe) threadId: string,
    ...
    const uploaded = await Promise.all(
      files.map(async (file) => {
        ...
        return { name: file.originalname, path: stored.path, sizeBytes: stored.sizeBytes };
      }),
    );
