...
export class AgentService {
  ...
  async createRun(
    ...
    try {
      ...
      void this.runService
        ...
        .catch((error) =>
          this.logger.error({ error, runId: run.id }, 'Agent run execution failed'),
        );
  ...
  async cancelRun(user: AuthenticatedUser, runId: string): Promise<AgentRunSummary> {
    ...
    const imageTask = await this.prisma.imageGenerationTask.findFirst({
      where: {
        agentRunId: runId,
        status: { in: ['PENDING', 'SUBMITTING', 'RUNNING', 'PERSISTING', 'CANCEL_REQUESTED'] },
      },
      select: { id: true },
    });
    if (imageTask) await this.imageGeneration.requestCancel(imageTask.id);
