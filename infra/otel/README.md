...
发布脚本将 Postgres / Redis / migrate / API / Web 作为业务主链路；Tempo 与 otel-collector 单独 best-effort 启动，其不 ready 不得使发布失败或长时间卡在维护页。

Tempo 只保留 Trace，`block_retention: 168h` 即 7 天；它保存的是诊断数据，不替代 PostgreSQL 业务备份。升级或回滚前按需单独备份 `tempo-data`，超过保留期的数据由 Tempo compactor 自动清理。若本地块过多导致 `/ready` 长期 503，可停掉 Tempo 后删除 `tempo-data` volume 再启动（仅丢失诊断 Trace）。
