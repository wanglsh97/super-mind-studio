...
The user interface SHALL always provide an Other custom-answer path. Every question in a batch MUST be answered before submission: a single-select question requires one option, a multi-select question requires one or more options, and selecting Other requires non-empty custom text. The server SHALL revalidate this rule, scope mutation to the run owner, and atomically settle a batch by stable `questionId`. The first valid settlement SHALL win; duplicate submissions SHALL return the original settled result.
