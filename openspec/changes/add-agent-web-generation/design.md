...
宽屏时 Website Artifact 与 `AgentConsolePanel` 在 `AgentPageShell` 内处于同一布局层级；结合外层 AppShell 导航形成“侧边栏 + 聊天区 + 网站产物区”三栏结构。产物区不是聊天卡片内部的浮层或绝对定位子面板，聊天列优先占用剩余空间，产物列约占 42% 并限制在 22–40rem，只用边线区分区域；窄屏时使用无圆角覆盖式面板。关闭后仍可从当前交付卡再次打开。新的成功覆盖会切换到最新产物，已覆盖卡不再打开旧预览或下载。
