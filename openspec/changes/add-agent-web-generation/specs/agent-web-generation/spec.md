...
#### Scenario: Artifact width is adjusted
- **GIVEN** the current Artifact workspace is shown beside the chat on a wide screen
- **WHEN** the user drags the divider or uses its keyboard controls
- **THEN** the Artifact width changes within its configured minimum/maximum and preserves the chat minimum usable width
