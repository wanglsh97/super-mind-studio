...
- 采用错误/超时/取消/failover/慢请求全量、正常成功请求 5% 的 tail sampling，并确保观测组件故障不影响模型调用、账单与请求生命周期终结。

## Capabilities

### New Capabilities

- `private-otel-observability`: 私有自建 Trace/Metric 采集、脱敏、采样、保留与业务日志关联。
...
- `infra/compose` 将新增 OTel Collector、Tempo 及其仅内网访问、7 天保留、采样与敏感字段删除配置；不修改 PostgreSQL 的业务真源职责。
