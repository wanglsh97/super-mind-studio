...
- 每个 Agent Thread 首次执行时创建一个可跨多轮 Run 复用的 OpenSandbox Linux 沙箱；默认最大生命周期和空闲清理上限均由 `SANDBOX_TIMEOUT_SECONDS=3600` 控制。每个 Run 绑定到 ready Sandbox 后会以 4 路并发异步增量预取当前用户所有已添加且已发布的候选 Skill，不阻塞 Run 启动；手动选择或模型调用 `skill` 时优先读取带完成标记的 Thread 本地包，不再访问 PostgreSQL 或 OSS，预取失败或本地缺失时才按当前权限签发短时只读 OSS URL 并同步补下载一次。
...
- `agent-context`: Prompt Composer 从“注入全部已启用 Skill”改为先通过 `<available_skills>` 注入候选目录，并在 `skill` 调用后通过 `<skill_content>` 加载完整 `SKILL.md`。
- `agent-tools`: 增加 `skill`、Shell 与文件工具，并将实际执行委托给当前 Thread 的 OpenSandbox，而非 NestJS 进程或宿主机。
