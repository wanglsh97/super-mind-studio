...
- 建立 Pino 结构化日志、PostgreSQL 业务记录、Redis 限流/健康状态和费用估算的可观测链路；补充自建 OpenTelemetry Trace/Metric 管道，用于诊断 Agent、模型流、Tool/MCP、PostgreSQL 与 Redis 的调用关系，不替代业务记录与账单真源。
...
- `observability-billing`: 完整 Prompt 请求日志、结构化运行日志、Token/费用记录和运行指标聚合，以及仅管理员可见的自建 OTel Trace 摘要与只读调用链。
...
- 新增 Nginx、Next.js、NestJS、PostgreSQL、Redis、内网 OTel Collector 与 Tempo 的单机 Docker Compose 拓扑和 ECS 运维配置；观测组件不开放公网端口。
