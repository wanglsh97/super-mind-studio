...
- Agent run 仅通过真实模型厂商 Adapter 提供生产能力；离线测试使用不参与 NestJS 注册的脚本夹具验证事件流、请求日志和计费记录。
...
- 提供阿里云 ECS 单机 Docker Compose + Nginx 部署方案，同时支持域名和公网 IP 访问；应用容器发布异常时由常驻 Nginx 返回静态维护页，而不向访客暴露代理错误页。
...
- `platform-foundation`: 工程骨架、共享契约、配置管理、数据库/缓存基础设施和健康检查。
...
- 文本模型没有真实厂商 API Key 时不暴露对应模型；离线单元与集成测试使用测试夹具，不把 Mock Chat 注册为生产 Adapter。
- 回滚以容器镜像版本、厂商 feature flag 和数据库向后兼容迁移为边界，已落库请求记录不删除。

## V1 Acceptance Boundary

本 change 的最低成功标准是：登录用户从根路径 Agent 工作台创建持久 thread/run，经 `@supermind/sdk` 调用 NestJS Agent API和已启用的真实厂商 Adapter，以可恢复事件流返回 reasoning、文本和终态，页面可正确展示，并在 PostgreSQL 中形成可被管理后台查询的请求日志与计费记录。独立 `/chat` 仅保留到 `/` 的兼容跳转，`/chat/compare` 与 `POST /api/v1/chat/completions` 不再属于产品契约。
