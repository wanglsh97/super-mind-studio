...
The API SHALL define reviewed Streamable HTTP MCP endpoints in release-controlled code and resolve credentials only from server-side environment variables. Each MCP Server MUST be disabled for a user until that user explicitly enables it. The Web Plugin page at `/plugin` MUST expose only sanitized server status and tool counts, and let users filter the catalog by category. A Server manifest MAY declare an explicit tool allowlist; when it does not, the Agent SHALL expose every safely discovered tool from that enabled Server, subject to the configured discovery limits. Neither the browser nor persisted Agent events may contain an MCP endpoint or credential.
