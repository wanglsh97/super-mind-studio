...
- [x] 2.20 在 Dashboard 增加今日 Token 概览、按实际模型的输入/输出/缓存/思考明细、三个月全站日历热力图及 Skill/Tool 聚合，以万为单位紧凑展示大值，并验证响应不泄露用户内容
...
- [x] 3.44 新增用户 Token 分析页，展示最近一年 GitHub 风格固定阈值总量热力图、阈值图例及日期精确值悬浮提示、每日 input/output/cache/reasoning 明细和实际模型总量图，并以万为单位紧凑展示大值
