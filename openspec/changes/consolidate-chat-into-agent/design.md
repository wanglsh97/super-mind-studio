...
- 保持旧书签和已有链接可恢复地跳转。
- 删除多模型对比及底层公开 Chat API/SDK，保留 Agent 与 Image/Prompt API/SDK。
- 不破坏登录、登出回跳和导航激活状态。

### Non-Goals

- 不把 `/chat/compare` 改写为 Agent 多 run 编排；直接退役该能力。
- 不删除模型 Adapter、Agent 模型调用计费或日志。
...
`/chat/compare`、SDK `chat.stream/compare` 和 `POST /api/v1/chat/completions` 一并删除，不迁移为 Agent 多 run。厂商 Chat Completions transport 仅作为 Adapter 实现细节保留，统一模型调用端口继续服务 Agent 和内部 Prompt。

### 删除旧页面专属 adapter

`agent-chat-adapter.ts` 只被旧 `/chat` 页面使用。移除页面后同时删除 adapter 及其测试，避免保留无人消费的浏览器内存聊天路径。Markdown 渲染和共享 thread UI 仍被 Agent 与 Prompt 使用，继续保留。

### 登录默认回跳到 Agent

`sanitizeUserReturnTo` 只允许 `/` 与 `/mcp`，缺失、`/chat/compare`、`/agent` 或其他非法值统一回退 `/`。
...
浏览器复核后将“新建会话”从高饱和主按钮降为浅色玻璃次级操作：保留 Logo 下方的位置和明确加号，但移除实色渐变、强阴影与旋转动效，避免压过当前页面内容。侧栏不再单独渲染 Skill 导航卡；`/skills` 入口移入用户头像菜单，与主题、管理后台和退出登录组成低频功能浮层。

## Risks / Trade-offs

- Agent 比旧 Chat 多一次 thread/run 持久化并受单用户 active run 约束，但换来刷新恢复、审计和扩展能力一致性。
- 删除对比能力会让原 `/chat/compare` 直接 404；这是用户确认的产品范围收敛。
...
- 用户认证 helper 测试确认 `/agent`、`/chat`、`/chat/compare`、`/image`、`/prompt` 和非法回跳均回退 `/`。
- 侧栏测试确认默认只返回最新 5 条对话，展开后返回全部记录；浏览器验证新建按钮位于 Logo 下方且旧 Agent 导航卡不存在。
- 浏览器验证“新建会话”为低对比次级样式，侧栏主区域不含 Skill 卡片，头像菜单中存在可用的 Skill 入口。
