...
- 删除 Agent Composer 的“模型对比”入口和 `/chat/compare` 页面，C 端只保留持久 Agent 对话能力。
...
- 删除公开 Chat completions API 和 `@supermind/sdk` 的 `chat.stream/compare`；保留 Image/Prompt API、模型 Adapter、数据库与管理员日志。
...
- `web-agent`: `/` 成为普通对话和工具型任务的唯一入口。
- `user-workspace-routing`: Agent-only 根工作台导航、登录回跳、旧 `/chat` 兼容行为及 `/agent`、Image、Prompt、API 展示路由退役。

## Impact

- `apps/web` 删除营销首页及旧 Chat、Compare、Image、Prompt、API 展示页面专属代码，把 Agent 页面迁移到根路径并更新导航与登录回跳。
- `spec/需求文档.md`、`spec/技术选型方案.md` 和 README 同步新的用户端信息架构。
- API/SDK 删除普通 Chat 契约，服务端模型基础设施收敛为 Agent 内部网关；数据库无 migration。
- 回滚时可恢复旧首页、`/agent`、`/chat`、`/chat/compare`、`/image`、`/prompt` 页面和导航；内部模型网关、Image/Prompt API 始终保留。
