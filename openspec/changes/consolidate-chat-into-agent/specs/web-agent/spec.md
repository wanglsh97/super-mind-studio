...
The system SHALL provide `/` as the canonical authenticated user surface for both ordinary model conversation and multi-step tool-assisted work. Each Agent thread SHALL use one enabled model instance that declares Agent/tool-calling capability. The selected model MUST remain immutable for that thread; selecting another model SHALL create another thread. The Agent surface SHALL expose the separate multi-model comparison experience as a sub-entry without changing comparison isolation semantics.
...
- **AND** returning from comparison opens `/`
